...
def costSolver(n):
	...
	cost_model += sum([ERcost[i] * Lvar[i] for i in Lvalues])


	# emissions constraint
	emit = 0
	for t in range(n): # index through the summation for clarity/ease
		highEmit = eh[t]* Fh[t]*(Kh*(1-float(t)/float(n)) + sum([Hvar[i] for i in Hvalues[:t]]) - sum([Lvar[i] for i in Lvalues[:t]]))
		lowEmit = el[t]* Fl[t]*(Kl + Kh*(float(t)/float(n))  - sum([Hvar[i] for i in Hvalues[:t]]) + sum([Lvar[i] for i in Lvalues[:t]]))
