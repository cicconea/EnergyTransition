...
def genData():
	...
	params["period"] = 10 # simulation length
...
def writeSolution(params, varDict, constraintDict, nameString=""):
	...
	f = open( "results/" + str(params["phi"])+ "_" + str(params["k"]) + "_" +nameString + "_constraints.csv", 'wb')
	writer = csv.writer(f)
	for key, value in constraintDict.items():
		writer.writerow([key, value])
	f.close()

	f = open( "results/" + str(params["phi"])+ "_" + str(params["k"]) + "_" + nameString + "_variables.csv", 'wb')
	writer = csv.writer(f)
	...
	f = open( "results/" + str(params["phi"])+ "_" + str(params["k"]) + "_" + nameString + "_checks.csv", 'wb')
	writer = csv.writer(f)
